$(document).ready(function () {
  ...
  $('button').on('click', function () {
    ...
    $('.places article').remove();
    const amenParam = { amenities: Object.keys(checkons) };
    $.ajax({
      type: 'POST',
      url: 'http://localhost:5001/api/v1/places_search/',
      data: JSON.stringify(amenParam),
      contentType: 'application/json',
      success: function (data) {
        for (let i = 0; i < data.length; i++) {
          $('section.places').append('<article><div class="title"><h2>' + data[i].name + '</h2><div class="price_by_night">' + data[i].price_by_night +
...
data[i].number_bathrooms + ' Bathroom</div></div><div class="description">' + data[i].description + '</div></article>');
        }
      }
    });
    console.log(amenParam);
  });
});
